...
async function main() {
  ...
  const approval = await request(app)
    .post(`/purchase-orders/${create.body.purchaseOrderId}/approval-decisions`)
  ...
  const fulfill = await request(app)
    .patch(`/purchase-orders/${create.body.purchaseOrderId}/status`)
  ...
  const partial = await request(app)
    ...
    .send({
      ...
      fulfillmentRecords: [{ lineItemId: create.body.lineItems[0].purchaseOrderLineItemId, quantity: 1, reference: 'ASN-1001', remarks: 'Initial shipment' }],
    });
  console.log('PARTIAL', partial.status, JSON.stringify(partial.body, null, 2));

  const history = await request(app).get(`/purchase-orders/${create.body.purchaseOrderId}/fulfillment-history`);
  console.log('HISTORY', history.status, JSON.stringify(history.body, null, 2));
